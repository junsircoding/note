...
#### 保险条款优化 ( 2022年4月 ～ 2022年6月)

集团内部的平台有一个保险条款管理的模块。该模块会根据用户在页面点选的内容生成一个初步的保险条款 Word 文档，之后会把这个文档进行 base64 编码，然后上传给服务。

服务对文档内容进行解析，对文档格式进行标准化，并且通过机器学习或者正则识别的方式对语句措辞进行校验和修改，最终返回一个比较文档和优化后的文档。

解析文档用的是 Python-docx 库，可以提取出文档里面的段落内容，段落格式，进而修改字体，以及对字体高亮等等操作。

这个项目需要合理地对条款文档的规则进行抽象，在保证解析的准确性的同时还要保证代码性能。机器学习部分需要编写对应的神经网络，包括 LSTM，RNN 等算法。
...
## 深圳中软国际科技服务有限公司 ( 2020年4月 ~ 2021年10月 ) 
